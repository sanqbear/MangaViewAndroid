...
public class StripAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public StripAdapter(Context context, Manga manga, Boolean cut, int width, Title title,
            ViewerActivity.InfiniteScrollCallback callback) {
        autoCut = cut;
                        ...
                        public void onResourceReady(@NonNull Bitmap bitmap, Transition<? super Bitmap> transition) {
                            ...
                            if (width > height) {
                                if (item.side == PageItem.FIRST) {
                                    if (reverse)
                                        holder.frame
                                                .setImageBitmap(Bitmap.createBitmap(bitmap, 0, 0, width / 2, height));
                                    else
                                        holder.frame.setImageBitmap(
                                                Bitmap.createBitmap(bitmap, width / 2, 0, width / 2, height));
                                } else {
                                    if (reverse)
                                        holder.frame.setImageBitmap(
                                                Bitmap.createBitmap(bitmap, width / 2, 0, width / 2, height));
                                ...
                                if (item.side == PageItem.FIRST) {
                                    ...
                                    holder.frame.setImageBitmap(
                                            Bitmap.createBitmap(bitmap.getWidth(), 1, Bitmap.Config.ARGB_8888));
                                }
                            }
