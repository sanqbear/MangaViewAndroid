...
public class Decoder {
    ...
    public Bitmap downSize(final Bitmap input, Float ratio) {
        ...
        Bitmap bitmap = Bitmap.createScaledBitmap(input, ((Float) (input.getWidth() * ratio)).intValue(),
                ((Float) (input.getHeight() * ratio)).intValue(), true);
        return bitmap;
    }
    ...
    private int newRandom(int index) {
        index++;
        double t = 100 * Math.sin(10 * (__seed + index)), n = 1000 * Math.cos(13 * (__seed + index)),
